        ...
        if($getFtvByLcID->num_rows() > 0){
			...
			foreach($getFtvByLcID->result() as $ftv){
				...
				$allfetival .= '<div class="col-lg-3 .col-xs-6 div-ftv-contain"><div class="div-ftv">
                    '.img(array('src'=>'user_uploads/thumbnail/original/'.$ftv->pho_source,'alt'=>$ftv->ftv_name,'class'=>'img-responsive img-thumbnail ftv-thumbnail')).'
                    <h4>'.$ftv->ftv_name.'</h4>
                    <p>'.character_limiter($ftv->ftv_detail, 200).'</p>';
                ...
                $allfetival .= form_submit('submit', 'Book','class="btn btn-info pull-right"');                            
