	  ...
	  $cus_transportation = unserialize($cus_transportation); 
		foreach ($cus_transportation as $transportation) {
			foreach ($transportation as $transport) {
				$extraproduct['departure'] = $transport['departure'];
            	$extraproduct['return_date'] = $transport['return_date'];
				?>
				<div class="panel panel-default">
					<div class="panel-heading">
				      <div class="panel-title">
				        ...
				        <span class="icon-list-alt pkadd clickDetailtp" data-url="<?php echo base_url(); ?>customize/tpdetail/<?php $transport['info']->tp_id; ?>/<?php echo $this->uri->segment(3); ?>" data-toggle='modal' data-target='.modalDetials' title="Details" style="float:right; margin-left:5px;"></span>
				      </div>
				    </div>
