    ...
    */
    public function submitFeedback(){
        if($this->session->userdata("subjectfeedback") == $this->input->post("fb_subject")){
            ...
            $insert['fb_date'] = $this->input->post("fb_date");
            //var_dump($insert);
...
Yours sincerely,";
    ...
    */
    ...
    $this->email->set_mailtype("html");
        $adminMail = MU_Model::getAdminEmail();
        if($adminMail == ""){
            $adminMail = "phansaorin@gmail.com";
        }
    $message = "<p>Firstname: ".$data['sub_fname'].
    "</p><p>Lastname: ".$data['sub_lname'].
    "</p><p>Email: ".$data['sub_email']."</p>";
        $this->email->clear();      
        $this->email->to($adminMail);
