...
// view subtransportation and extraproduct
    ...
    */
    ...
    public function actdetail($act_id, $bk_id){
    if ($this->check_user_session()) {
		$cuscon_id = $this->mod_customize->getCustomizeConjectionID($bk_id);
        ...
        $this->showDetailsAct($activity[$act_id]);
    }
}
    // function to view detial of accommodation
    public function accdetail($acc_id, $bk_id){
    if ($this->check_user_session()) {
		$cuscon_id = $this->mod_customize->getCustomizeConjectionID($bk_id);
        ...
        $this->showDetailsAcc($accommodation[$acc_id]);
    }
}
    ...
    if ($this->check_user_session()) {
		//$subID = $this->mod_customize->getObjectClassOfTable('acti_calendar', 'activities_id', $details['info']->act_id);
		var_dump($details['info']->act_id);
		var_dump($details); die;
		$object = $this->mod_customize->get_info_of_main_obj('accommodation', 'acc_id', $id);
