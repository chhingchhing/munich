...
class mod_fepackage extends MU_Model{
    ...
    */
    // 24th june 2014
    public function getallpackages($ppl, $ftv, $lc ,$dpdate, $rtdate = false){
    	$today = date("Y-m-d");
        $query = $this->db->select('*')         
            ->where('package_conjection.pkcon_lt_id', $lc)
            ->where('package_conjection.pkconl_ftv_id', $ftv)
            ->where('package_conjection.pkcon_actualstock >=', $ppl)
            ->where('package_conjection.pkcon_start_date >=', date("Y-m-d", strtotime($dpdate)))
            // ->or_where('package_conjection.pkcon_end_date >', date("Y-m-d", strtotime($dpdate)))
