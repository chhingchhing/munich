...
jQuery(function(){
	...
	var vwp = 0;
	jQuery('.view_package select, .view_package input[type="text"], .view_package textarea').prop("disabled", true);
	jQuery('.view_enable_package').bind("click", function(){
		if( vwp == 0 ){ 
			jQuery('.view_package select, .view_package input[type="text"], .view_package textarea').prop("disabled", false);
			jQuery(".view_enable_package").text("Disable from Editing");
			vwp = 1;
		}else if( vwp == 1 ){ 
			jQuery('.view_package select, .view_package input[type="text"], .view_package textarea').prop("disabled", true);
			jQuery(".view_enable_package").text("Enable for editing");
			vwp = 0;
		}
	});
	// while submit form...
	jQuery(".view_package").bind("submit", function(){
		jQuery('.view_package select, .view_package input[type="text"], .view_package textarea').prop("disabled", false);
	});
